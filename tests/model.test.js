...
const brick=(overrides={})=>({id:'a',part:'brick-2x4',color:'#c73536',x:0,y:0,z:0,rotation:0,...overrides});
const project=pieces=>({format:'bricklab',version:1,name:'Test',size:32,pieces});

test('rotation swaps footprint and catches the new collisions and base boundary',()=>{
  const a=brick({rotation:90});assert.deepEqual(dimensions(a),{w:4,d:2,h:3});
  assert.equal(placementError(a,[],32),null);
  assert.match(placementError({...a,x:29},[],32),/fuera/);
  assert.equal(collides(a,brick({id:'b',x:3,z:1})),true);
  assert.equal(collides(a,brick({id:'b',x:4,z:0})),false);
});
test('bricks stack at three plates and plates stack at one',()=>{
  ...
  const plate=brick({part:'plate-2x4'});
...
test('smooth tiles cannot support an attached brick, floating mode is explicit',()=>{
  const tile=brick({part:'tile-2x4'});
...
test('bridge, hanging attachment, detached assemblies and support deletion',()=>{
  const base=brick({part:'brick-2x2'});
  const bridge=brick({id:'b',y:3});
  const hanging=brick({id:'c',part:'plate-2x2',z:2,y:2});
...
test('moving ignores itself but still rejects overlaps with other pieces',()=>{
  const a=brick(),b=brick({id:'b',x:3});
...
test('negative, fractional, non-finite and excessive positions are rejected',()=>{
  for(const pos of [{x:-1},{y:-1},{z:NaN},{x:.5},{y:Infinity},{y:300}])assert.ok(placementError(brick(pos),[],32));
});
...
test('import rejects unknown/prototype parts, malformed schema, duplicated ids and collisions',()=>{
  for(const bad of [null,{}, {...project([]),version:2},{...project([]),size:100},{...project([]),name:''},project([brick({part:'toString'})]),project([brick({part:'unknown'})]),project([brick({color:'red'})]),project([brick({rotation:45})]),project([brick(),brick()]),project([brick(),brick({id:'b'})]),project([brick({x:'1'})]),project(Array(2001).fill(brick()))])assert.throws(()=>parseProject(bad));
...
test('inventory groups type and color; dimensions include rotated footprints and height',()=>{
  const pieces=[brick(),brick({id:'b',x:2}),brick({id:'c',x:4,color:'#ffffff',rotation:90,y:3})];
  const rows=inventory(pieces);assert.equal(rows.length,2);assert.equal(rows.find(r=>r.color==='#c73536').count,2);
  assert.deepEqual(bounds(pieces),{x:0,y:0,z:0,w:8,h:6,d:4});assert.equal(bounds([]).h,0);
});
