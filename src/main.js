...
import {MECHANICAL_PARTS,mechanicalJoints} from './mechanical.js';
...
let precisionOpen=false,precisionKey=null,grabAnchor='center',connectionPreference='auto';
...
$('#app').innerHTML=`
  ...
  <main class="workspace">
    ...
    <section class="stage" id="stage" aria-label="Editor 3D">
      ...
      <details class="controls-legend" id="controls-legend">
        <summary>${icon('help-circle')}<span>Botones y controles</span>${icon('chevron-down')}</summary>
        <div class="legend-content"><h2>Herramientas</h2><dl>${[['plus','B','Construir'],['mouse-pointer-2','V','Seleccionar pieza'],['move','M','Mover pieza'],['boxes','G','Conjunto conectado'],['paintbrush','P','Pintar'],['eraser','X','Borrar']].map(([i,key,label])=>`<div><dt>${icon(i)}<kbd>${key}</kbd></dt><dd>${label}</dd></div>`).join('')}</dl><h2>Colocar con precisión</h2><dl>${[['Flechas','Mover X/Z'],['RePág / AvPág','Subir / bajar'],['Shift + movimiento','Paso de 0,5'],['R','Girar 90°'],['A','Centro / esquinas'],['L','Fijar / seguir ratón'],['Enter','Colocar o confirmar'],['Esc','Cancelar movimiento']].map(([key,label])=>`<div><dt><kbd>${key}</kbd></dt><dd>${label}</dd></div>`).join('')}</dl><h2>Cámara y edición</h2><dl>${[['Arrastrar','Orbitar cámara'],['Rueda','Acercar / alejar'],['Botón derecho','Desplazar cámara'],['F','Centrar modelo'],['Ctrl/⌘ Z','Deshacer'],['Ctrl/⌘ Shift Z','Rehacer'],['Ctrl/⌘ D','Duplicar pieza'],['Supr','Eliminar pieza'],['Ctrl/⌘ S','Guardar copia']].map(([key,label])=>`<div><dt><kbd>${key}</kbd></dt><dd>${label}</dd></div>`).join('')}</dl><p>En móvil: un dedo orbita; dos dedos acercan o desplazan. Los botones de precisión permiten ajustar cada eje.</p></div>
      </details>
...
function refreshCandidate(){
  ...
  const mechanicalMatch=project.pieces.some(p=>p.id!==moving?.id&&mechanicalJoints(candidate,p).length);
  ...
  renderPrecision();
  setStatus(candidateError||`${mechanicalMatch?'Asa y clip alineados · ':''}${placementCursor.locked?'Posición fijada':'Colocar'} · X ${candidate.x} · Y ${candidate.y} · Z ${candidate.z} · Enter coloca`,!!candidateError);
}
...
function hoverAssembly(hit){
  ...
  const next=candidateFromHit(anchor,hit,{connection:connectionPreference,pieces:assembly.others,size:project.size,allowFloating:true});
...
function requestAssemblyPlacement(){
  ...
  openDialog(assemblySnap?'¿Unir los conjuntos aquí?':'¿Colocar el conjunto aquí?',`<p class="dialog-intro">Se moverán ${assembly.ids.size} piezas juntas. ${assemblySnap?'Los conectores quedan alineados con la otra construcción.':'Se conservarán las posiciones relativas de todas las piezas.'} Puedes deshacer el movimiento completo después.</p><div class="assembly-finish"><button data-action="assembly-confirm" class="primary">Sí, colocar aquí</button><button data-action="close-dialog">Seguir ajustando</button></div>`);
}
...
function hover(hit){
  ...
  candidate=placementCursor.follow(candidateFromHit({...base,rotation},hit,{anchor:grabAnchor,connection:connectionPreference,pieces:project.pieces,size:project.size,allowFloating}));
  refreshCandidate();
}
...
document.addEventListener('change',e=>{
  if(e.target.id==='connection-preference'){connectionPreference=e.target.value;placementCursor.resume();hover(lastHit);renderPrecision();}
...
document.addEventListener('keydown',e=>{
  if(e.target.closest('input,textarea,select,[contenteditable="true"],#controls-legend')||$('#dialog').open)return;
